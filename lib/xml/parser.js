...
function getNormalOccurenceEventData(nextEvent, eventData, vevent) {
    ...
    if (dtend != null) {
        ...
        duration = vevent.getFirstPropertyValue("duration");
    }
    const origTimezone = vevent.getFirstPropertyValue('dtstart')?vevent.getFirstPropertyValue('dtstart').timezone : undefined;
    ...
    return {
        ...
        location,
        description,
        start: getCorrectDate(dtstart, origTimezone),
        end: getNormalizedEndDate(getCorrectDate(dtend, origTimezone), duration),
        duration,
        type: { recurring: true, edited: false },
        createdAt: getCorrectDate(vevent.getFirstPropertyValue("created")),
        organizer: getOrganizer(vevent),
        attendies: getAttendies(vevent),
    };
}
...
function getAttendies(vevent) {
    let attendies = {}
    const attendiesProps = vevent.getAllProperties("attendee")
    attendiesProps.forEach(attendee => {
        const partstat = attendee.getParameter('partstat')
        if (!attendies[partstat]) {
            attendies[partstat] = new Set()
        }
        const mailto = attendee.getFirstValue("mailto")
        if (mailto) {
            attendies[partstat].add(mailto.toLowerCase())
        }
    });
    return attendies;
}
...
function getOrganizer(vevent) {
    let organizer = '';
    const organizerObj = vevent.getFirstProperty("organizer");
    if (organizerObj) {
        organizer = organizerObj.getParameter("cn");
        if (!organizer) {
            organizer = organizerObj.getFirstValue();
        }
    }
    return organizer;
}

function getCorrectDate(vdate) {
    return vdate ? vdate.toJSDate() : new Date();
}

const stripDescr = new RegExp(/(^(?:DESCRIPTION)|(?:X-ALT-DESC))[\s\S]*?(?=^\w[\w-]+)/, "gm")
...
function parseEvents(xml) {
    ...
    return parseXMLString(xml, PARSE_XML_CONFIG).then((result) => {
        ...
        parsed.forEach((event) => {
            ...
            let jcalData = {};
            try {
                jcalData = ICAL.parse(calendarData);
            } catch (err) {
                console.log("===============================================")
                console.log("Failed to parse event, err: " + err + "\n" + calendarData);
                const strippedCalData = calendarData.replace(stripDescr, "$1:\n")
                try {
                    jcalData = ICAL.parse(strippedCalData);
                } catch(newErr) {
                    console.log("----------------------------------------------")
                    console.log("Failed to parse stripped event, err: " + newErr + "\n" + strippedCalData);
                    return;
                }
            }
            ...
            // ignore calendar-data with no events
            if ( !vevent ) {
                return;
            }
            ...
            if (icalEvent.isRecurring()) {
                ...
                // Use just two weeks offset from the current date.
                for ( ; ; ) {
                    const nextOccuranceTime = expand.next();

                    if (!expand.complete) {
                        let nextTime = moment.unix(nextOccuranceTime.toUnixTime());
                        const diff = moment.duration(nextTime.diff(curDate)).asWeeks();
                        if (diff<=-weeksOffset) {
                            continue;
                        }
                        else if (diff>weeksOffset) {
                            break;
                        }
                    } else {
                        break;
                ...
                eventData = {
                    ...
                    location: vevent.getFirstPropertyValue("location"),
                    description: vevent.getFirstPropertyValue("description"),
                    start: getCorrectDate(dtstart),
                    end: getNormalizedEndDate(getCorrectDate(dtend), duration),
                    duration,
